<template>
	<view class="contain-box">
		...
		<view class="img-list-box">
			<view class="row-box" v-if="item.length>0" v-for="(item, index) in getImgList(article_info.imgList)" :key="index">
				<view class="column-box" v-for="(bitem, bindex) in item" :key="bindex" @click="imgPreview(bitem.url)">
					<view class="img-single">
						<u-image width="100%" height="100%" border-radius="10rpx" :src="bitem.url"></u-image>
					</view>
				</view>
				<view class="column-box" v-if="" v-for="(citem, cindex) in (3 - item.length)" :key="cindex">
					<view class="img-single"></view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		props:{
			article_info: {
				...
				default(){
					return {
						id: 0,
						head_portrait: '/static/wechat_pay.jpg',
						nickname: '坏人今夜不归来',
						time: '01-15 20:37',
		...
		methods: {
			imgPreview(url){
				console.log("点击了预览",url)
				uni.previewImage({
					indicator:"number",
					loop:true,
					urls: [url,url]  //设置url数组可以循环切换预览图
				})
			},
			getImgList(list){   //将图片包装成每3个一行
				let newList = [[]]  //此时长度为 1
