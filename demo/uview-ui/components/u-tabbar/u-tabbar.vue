<template>
	<view v-if="show" class="u-tabbar" @touchmove.stop.prevent="() => {}">
		<view class="u-tabbar__content safe-area-inset-bottom" :style="{
			...
			<view v-if="midButton" class="u-tabbar__content__circle__border" :class="{'u-border': borderTop,}" :style="{
				backgroundColor: bgColor,
				left: midButtonLeft
			}">
			</view>
...
<script>
	export default {
		props: {
			...
			// 通过v-model绑定current值
			value: {
				type: [String, Number], //string 或者 number 类型
				default: 0
			},
			// 整个tabbar的背景颜色
			bgColor: {
		...
		methods: {
			...
			// 切换tab
			switchTab(index) {
				// 发出事件和修改v-model绑定的值，通过change把index传给父组件，也就是index.vue
